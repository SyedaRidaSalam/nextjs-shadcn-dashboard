  ...
  return (
    <div className="">
       ...
       <div className="flex flex-col gap-2">
        ...
        {list.map(elem => (
            <Card key={elem.id} className="flex-row items-center justify-between gap-4 p-4 mb-3">
                <div className="w-12 h-12 rounded-sm relative overflow-hidden">
                    <Image src={elem.image} alt={elem.title} fill className="object-cover" />
                </div>
                <CardContent className="p-0 flex-1">
                    <CardTitle className="text-sm font-medium">{elem.title}</CardTitle>
                    <Badge variant='secondary'>{elem.badge}</Badge>
                </CardContent>
                <CardFooter className="p-0">{elem.count/1000}k</CardFooter>
            </Card>
        ))}
        </ScrollArea>
       </div>
